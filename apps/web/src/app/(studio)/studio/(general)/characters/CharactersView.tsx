          ...
          <table className="border-border w-full border-collapse text-left text-sm">
            ...
            <tbody>
              {characters.map((character, index) => (
                <tr key={character.id} className="border-border hover:bg-muted/50 cursor-pointer border-b" onClick={() => window.location.href = `/studio/characters/${character.id}`}>
                  <td className="px-2">
                    <Checkbox
                  ...
                  <td className="flex items-center gap-3 py-4">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-500">{displaySpecies(character.species ?? "")}</div>
                    </div>
                  </td>
                  <td className="py-4">
                    {character.adoptionStatus ? (
                      <div className="text-purple-600 text-sm">
                        Adopted on {new Date(character.adoptionStatus.adoptionDate).toLocaleDateString()}
                      </div>
                    ) : (
                      <>
                        <div className="text-purple-600 text-sm">
                        ...
                        <div className="font-medium">{new Date(character.createdAt).toLocaleDateString()}</div>
                      </>
                    )}

                  </td>
                  <td className="py-4 font-medium">{character.owner.handle}</td>
                  <td className="py-4">
