...
export default function MoveArtworkMenu({
  artwork,
  folders,
  onMoved,
}: {
  ...
  const handleAssign = async (folderId: string | null) => {
    try {
      await assignArtworkToFolder(artwork.id, folderId)
      onMoved?.(artwork.id, folderId)
    } catch (error) {
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={`Move ${artwork.title ?? "artwork"} to folder`}
          onClick={(e) => {
            e.preventDefault()
            e.stopPropagation()
          }}
